<?php
    ...
    {
        case 0:
            $query = "INSERT INTO G_Ulink (GU_uid,GU_gid,GU_level) VALUES ('{$ID}','{$gId}','A')";
            mysqli_query($link, $query) or die(mysqli_error($link)); 
            header("location:groups.php?id=".$gId);
        break;
        
        default:
            $query = "UPDATE G_Ulink SET GU_uid={$ID},GU_gid={$gId},GU_level='A' WHERE GU_uid='{$ID}' AND GU_gid='{$gId}'";
            mysqli_query($link, $query) or die(mysqli_error($link)); 
            header("location:groups.php?id=".$gId);
        break;
    }
